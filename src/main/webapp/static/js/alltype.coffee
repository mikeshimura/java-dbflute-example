...
$w.handleChange = (e) ->
  ...
  if e.target.type == "checkbox"
    $c.handleChange(jsx,e.target.name,e.target.checked)
  else 
...
$w.handleClick = (e) ->
  ...
  if (ids[0] == "loginrow")
    logintemp={login:jsx.state.alltype}
    selRow = Number(ids[2])
...
$w.formSearch = (jsx) ->
  ...
  $w.flux.actions.$c_rcd_fetch(jsx.state.alltype,jsx.state.form,"alltype",criteria)
$w.formUpdate = (jsx) ->
  form = jsx.state.form
  res = ""
  if form.id==""
  ...
  if res.length > 0
    ...
    return
  $w.flux.actions.$c_rcd_update(jsx.state.alltype,jsx.state.form,"alltype")
$w.formDelete = (jsx) ->
...
$w.formDeleteCfm = (jsx) ->
  $w.flux.actions.$c_rcd_delete(jsx.state.alltype,jsx.state.form,"alltype")
...
$w.rcdStore.on("rcdComplete_alltype", ->
