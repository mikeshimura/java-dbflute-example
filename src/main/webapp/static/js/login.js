...
(function() {
  ...
  $w.login_formUpdate = function() {
    return $w.flux.actions.$c_rcd_update($w.app.state.login, $w.app.state.login_form, "login");
  };

  $w.login_formDelete = function() {
    if ($w.app.state.login_form.id === "") {
      $w.flux.actions.$c_rcd_delete_id_blank();
      return;
    }
    return $w.flux.actions.$c_deleteCfmShow($w.login_formDeleteCfm);
  };

  $w.login_formDeleteCfm = function() {
    return $w.flux.actions.$c_rcd_delete($w.app.state.login, $w.app.state.login_form, "login");
  };

  $w.login_formClear = function() {
    var formtemp;
    formtemp = {
  ...
  $w.login_scroll = function() {
    return $w.getDom("login_tableHead").scrollLeft = $w.getDom("login_tableBody").scrollLeft;
  };

  $w.login_onscroll = function() {
    return $w.getDom("login_tableBody").onscroll = $w.login_scroll;
  };
